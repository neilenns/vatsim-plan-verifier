...
| Cruise altitude | The first line shows the initial altitude to assign, if known. If "CVS" is shown in front of the altitude then the initial altitude should be issued with the "Climb via SID" phraseology. The second line shows when the pilot should expect to get their final cruise altitude, in minutes. If this information is shown in parenthesis, e.g. `(5 minutes)`, then the time is published on the chart and does not have to be given to the pilot as part of the clearance unless the cruise altitude was amended. |
